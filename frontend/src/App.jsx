...
export default function App() {
  ...
  function navigate(href) {
    ...
    const reactPath = ['/ERP/main.do', '/ERP/approval/list.do', '/ERP/approval/view.do']
  ...
  return (
    <div className="app-shell">
      ...
      <div className="floating-actions">
		...
		<button className="floating-button chat-button" onClick={() => setChatOpen((open) => !open)} aria-label={chatOpen ? 'AI 챗봇 닫기' : 'AI 챗봇 열기'}><Icon name={chatOpen ? 'close' : 'chat'} /></button>
      </div>
  ...
  return (
    <aside className={`sidebar ${open ? 'open' : ''}`}>
      ...
      <nav className="main-nav" aria-label="주요 메뉴">
        ...
        {items.map((item) => (
          <a key={item.label} className={`nav-link ${isActive(item.href) ? 'active' : ''}`} href={item.href} onClick={(event) => { if (item.href.includes('/main.do') || item.href.includes('/approval/list.do')) { event.preventDefault(); navigate(item.href) } }}>
    ...
    <>
      ...
      <section className="legacy-status-panel" aria-label="오늘의 결재 현황">
        ...
        <div className="stat-grid">
          {stats.map(([label, value, tone, mode]) => <button key={label} className={`stat-card ${tone}`} onClick={() => mode ? navigate(`${BASE}/approval/list.do?mode=${mode}`) : window.location.assign(`${BASE}/approval/recv.do`)}><span>{label}</span><strong>{value}<small>건</small></strong></button>)}
        </div>
      </section>
      <section className="legacy-dashboard-grid">
        <LegacyCalendar />
        <article className="surface notice-panel">
          <div className="section-heading"><h2>공지사항 &gt;</h2><a href={`${BASE}/notice/list.do`}>전체 보기</a></div>
          <div className="notice-list">
            {dashboard.notices.length ? dashboard.notices.map((notice) => <a key={notice.noticeNo} href={`${BASE}/notice/view.do?notice_no=${notice.noticeNo}`}><span className={`notice-dot ${notice.important ? 'important' : ''}`} /> <strong>{notice.title}</strong><time>{notice.date}</time></a>) : <EmptyState title="새로운 공지가 없습니다." />}
          </div>
        </article>
      </section>
    </>
  ...
  return (
    <>
      <section className="page-heading">
        ...
        <a className="primary-button" href={`${BASE}/approval/write.do`}><Icon name="plus" /> 새 결재 작성</a>
      </section>
  ...
  return (
    <>
      ...
      <article className="surface document-surface">
        ...
        {(data.canEdit || data.canDelete) && <div className="document-actions">{data.canDelete && <button className="text-button danger" onClick={remove}><Icon name="trash" /> 삭제</button>}{data.canEdit && <a className="secondary-button" href={`${BASE}/approval/modify.do?approval_no=${item.approvalNo}`}><Icon name="edit" /> 수정</a>}</div>}
      </article>
    </>
  )
}
